...
const Cart = () => {
        ...
        try {
            ...
            if (data.rows && data.rows[0] && data.rows[0].elements && data.rows[0].elements[0]) {
                ...
                if (element.status === 'OK') {
                    ...
                    // Check if distance is an object with value, or just a number, or string
                    if (element.distance && typeof element.distance.value === 'number') {
                        ...
                        console.warn('Unexpected distance format:', element.distance);
                        // Try parsing if it's a string like "5 km" (unlikely for API but possible)
                        distanceInKm = calculateDistanceHaversine(areaCenter.latitude, areaCenter.longitude, lat, lng);
                    }
                    console.log(`Ola Maps Distance: ${distanceInKm} km`);
                } else {
                    console.warn('Ola Maps Distance Matrix returned non-OK status, falling back to Haversine');
                    distanceInKm = calculateDistanceHaversine(areaCenter.latitude, areaCenter.longitude, lat, lng);
                }
            } else {
                console.warn('Invalid Ola Maps Distance Matrix response, falling back to Haversine');
                distanceInKm = calculateDistanceHaversine(areaCenter.latitude, areaCenter.longitude, lat, lng);
            }
    ...
    const handleLocationInput = async (e, setFieldValue, pincode) => {
                                ...
                                <div className="relative" ref={suggestionsRef}>
                                    ...
                                    <div className="flex gap-2">
                                        <Field
                                            ...
                                            autoComplete="off"
                                            onKeyUp={(e) => handleLocationInput(e, setFieldValue, values.pincode)}
                                            onBlur={(e) => {
                                                handleBlur(e);
